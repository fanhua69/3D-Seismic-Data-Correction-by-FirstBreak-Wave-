...
bool CMemEquation::Turn()
{
	...
	CProDlg dlg;
	dlg.Create();
	dlg.SetWindowText ("Turning Equation");
	dlg.SetStatus("Turned:");
	...
	int nShowStep=(double)m_lRowNum/100.0;
	for(long Row=0;Row<m_lRowNum;Row++){
		if(Row%nShowStep==0){
			dlg.SetPos(Row/nShowStep);
		}
...
{
	double x1=0,y1=100,x2=m_lColNum*0.001,y2=1;
	double a=(x1*y1-x2*y2)/(y2-y1);
	double b=y1*(x1+a);
	CString sStatus;

	CProgressDlg dlg;
	dlg.m_nTotal=m_lColNum;
	dlg.Create();
	dlg.SetRange(0,100);	
	...
	long nCheckLoop=1,nMidCheckLoop=4,nLongCheckLoop=8,nUltraLongCheckLoop=15;
	long nProgressCheckLoop=1;
	...
	while(true){		
		...
		if(nLoop>18&&nLoop<600&&nLoop%nMidCheckLoop==0){
			for(i=0;i<m_lColNum;i++){
		...
		if(nLoop>50&&nLoop<500&&nLoop%nLongCheckLoop==0){
			for(i=0;i<m_lColNum;i++){
		...
		if(nLoop>54&&nLoop<400&&nLoop%nUltraLongCheckLoop==0){
			for(i=0;i<m_lColNum;i++){
